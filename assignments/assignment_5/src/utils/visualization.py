...
def plot_review_length_performance(
    ...
    transformer_correct = [pred == true for pred, true in zip(transformer_preds, sentiments)]
    ...
    for bin_assignment, b_correct, t_correct in zip(bin_assignments, baseline_correct, transformer_correct):
    ...
    for bin_label in bin_labels:
        if bin_data[bin_label]["count"] > 0:
            ...
            baseline_acc.append(sum(bin_data[bin_label]["baseline_correct"]) / bin_data[bin_label]["count"])
            transformer_acc.append(sum(bin_data[bin_label]["transformer_correct"]) / bin_data[bin_label]["count"])
...
def create_confusion_matrices(
    ...
    plot_confusion_matrix(
        cm=cm_transformer,
        labels=["Negative", "Positive"],
        output_path=figures_dir / "confusion_matrix_transformer.png",
...
def plot_confusion_matrices_side_by_side(
    ...
    cm_transformer = cm_transformer.astype('float') / cm_transformer.sum(axis=1)[:, np.newaxis]
    
    # Create subplots
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 7))
    
    # Plot logistic regression confusion matrix
    disp1 = ConfusionMatrixDisplay(confusion_matrix=cm_lr, display_labels=["Negative", "Positive"])
    ...
    disp2 = ConfusionMatrixDisplay(confusion_matrix=cm_transformer, display_labels=["Negative", "Positive"])
...
def visualize_results(
    ...
    if "logistic_regression" in results and "transformer" in results:
        ...
        if test_dataset is not None and "logistic_regression_preds" in results and "transformer_preds" in results:
