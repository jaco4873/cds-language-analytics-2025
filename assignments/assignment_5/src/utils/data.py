...
def load_and_prepare_imdb(
    ...
    test_set = imdb_dataset["test"].shuffle(seed=random_state).select(range(test_sample_size))
    
    # Ensure stratification by sampling equally from each class
    train_pos = train_full.filter(lambda x: x["sentiment"] == 1).select(range(train_sample_size // 2))
    train_neg = train_full.filter(lambda x: x["sentiment"] == 0).select(range(train_sample_size // 2))
    ...
    logger.info(f"Dataset: train={len(train_set)}, val={len(val_set)}, test={len(test_set)}")
    ...
    logger.info(f"Train sentiment distribution: 0 (negative): {train_neg_count}, 1 (positive): {train_pos_count}")
