...
class NgramModel:
    def __init__(self, name: str, n_gram_size: int = 2, *, use_smoothing: bool = True, use_stupid_backoff: bool = True):
    ...
    def conditional_probability_distribution(self, history: tuple[str, ...]) -> dict[str, float]:
        ...
        if history not in self.model:
            if self.use_stupid_backoff and self.backoff_models:
                logger.debug(f"Backoff: Using {len(history)-1}-gram model for unseen history {history}")
                shorter_hist = history[1:] if len(history) > 0 else tuple()
                backoff_dist = self.backoff_models[0].conditional_probability_distribution(shorter_hist)
                return {w: p * settings.BACKOFF_ALPHA for w, p in backoff_dist.items()}
            # Fallback to uniform distribution
            logger.debug(f"Fallback: Using uniform distribution for unseen history {history}")
    ...
    def generate(self, seed: tuple[str, ...] | None = None, 
                tokens: int = 25,
                top_k: int | None = None,
