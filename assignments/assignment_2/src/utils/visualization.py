...
def prepare_class_data(df, value_cols):
...
def create_accuracy_comparison(df, results_dir):
...
def create_f1_comparison(df, results_dir):
...
def create_precision_recall_comparison(df, results_dir):
